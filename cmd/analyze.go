...
func analyze(auctionName string) error {
	aiDir := filepath.Join(auctionName, "05_AI_Analysis_Photos")

	entries, err := os.ReadDir(aiDir)
	if err != nil {
		return fmt.Errorf("Error reading AI analysis folder: %v\n", err)
	}
	...
	if err != nil {
		return fmt.Errorf("SFTP upload failed: %v\n", err)
	}
	...
	if err != nil {
		return fmt.Errorf("Error creating TSV: %v\n", err)
	}
	defer f.Close()
	...
	writer.Flush()
	fmt.Println("Analysis complete - lots.tsv written successfully.")
