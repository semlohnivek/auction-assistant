...
func createAuctionScaffold(auctionName string, lotCount int) error {
	...
	for i := 1; i <= lotCount; i++ {
		...
		if err != nil {
			return fmt.Errorf("Error generating barcode for %s: %v\n", lotName, err)
		}
		codeScaled, err := barcode.Scale(code, 400, 100)
		if err != nil {
			return fmt.Errorf("Error scaling barcode for %s: %v\n", lotName, err)
		}

		barcodePath := filepath.Join(lotDir, fmt.Sprintf(barcodeFilename))
		file, err := os.Create(barcodePath)
		if err != nil {
			return fmt.Errorf("Error creating barcode file %s: %v\n", barcodePath, err)
		}
		defer file.Close()
		if err := png.Encode(file, codeScaled); err != nil {
			return fmt.Errorf("Error encoding barcode PNG for %s: %v\n", lotName, err)
		}
	}
